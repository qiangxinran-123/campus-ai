...
1. 打开 `http://127.0.0.1:8000`，确认看到 CampusAI 前端首页。
2. 在左侧点击课程，确认右侧资料列表随课程切换。
3. 在“上传学习资料”区域选择 `.txt` 或 `.md` 文件，点击“上传并提取文本”。
4. 确认新资料出现，并展示“已提取文本”、文本预览和字符数。
5. 在“搜索资料”区域输入关键词，点击“搜索当前课程”。
6. 点击某条资料的“生成摘要”，确认页面出现摘要状态和摘要内容。
7. 点击“生成卡片”，确认页面展示 3 张学习卡片。
8. 点击“清除搜索”，回到当前课程的完整资料列表。

讲解词：

> V0.2 增加了原生 HTML、CSS 和 JavaScript 页面。页面直接调用现有 FastAPI API，没有引入 Vue、React 或复杂构建工具，便于初学者理解和现场演示。
## 4. 查看课程列表
